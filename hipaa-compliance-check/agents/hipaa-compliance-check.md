...
description: |
  Use this agent to run a Kiteworks content-governance scan against HIPAA, checking only what a file-sharing platform can observe (fit tier: Strong -- see the skill for what's in and out of scope), and -- on confirmation -- write a CSV + txt/pdf report. Single-phase: no separate apply agent.

  <example>
  Context: User wants to know their HIPAA exposure before an audit or review.
  user: "Check the Legal folder against HIPAA"
  assistant: "Running the hipaa-compliance-check agent against that folder -- I'll be upfront about what this can and can't actually verify for HIPAA, then offer to save a report."
  <commentary>
