...
description: |
  Use this agent to run a Kiteworks content-governance scan against LGPD (Brazil), checking only what a file-sharing platform can observe (fit tier: Strong -- see the skill for what's in and out of scope), and -- on confirmation -- write a CSV + txt/pdf report. Single-phase: no separate apply agent.

  <example>
  Context: User wants to know their LGPD (Brazil) exposure before an audit or review.
  user: "Check the Legal folder against LGPD (Brazil)"
  assistant: "Running the lgpd-compliance-check agent against that folder -- I'll be upfront about what this can and can't actually verify for LGPD (Brazil), then offer to save a report."
  <commentary>
