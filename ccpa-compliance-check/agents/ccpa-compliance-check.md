...
description: |
  Use this agent to run a Kiteworks content-governance scan against CCPA/CPRA, checking only what a file-sharing platform can observe (fit tier: Strong -- see the skill for what's in and out of scope), and -- on confirmation -- write a CSV + txt/pdf report. Single-phase: no separate apply agent.
  ...
  assistant: "Running the ccpa-compliance-check agent against that folder -- I'll be upfront about what this can and can't actually verify for CCPA/CPRA, then offer to save a report."
  <commentary>
