...
description: |
  Use this agent to run a Kiteworks content-governance scan against SOC 2, checking only what a file-sharing platform can observe (fit tier: Light -- see the skill for what's in and out of scope), and -- on confirmation -- write a CSV + txt/pdf report. Single-phase: no separate apply agent.

  <example>
  Context: User wants to know their SOC 2 exposure before an audit or review.
  user: "Check the Legal folder against SOC 2"
  assistant: "Running the soc2-compliance-check agent against that folder -- I'll be upfront about what this can and can't actually verify for SOC 2, then offer to save a report."
  <commentary>
